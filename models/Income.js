...
const IncomeSchema = new mongoose.Schema(
  {
    ...
    incomeType: {
      type: String,
      enum: ['Sales', 'Services', 'Interest Income', 'Rental Income', 'Dividend Income', 'Other Income'],
      required: true,
    },
    customerId: {
...
// Generate income number before save
IncomeSchema.pre('save', async function() {
  if (this.isNew && !this.incomeNumber) {
    try {
      ...
      this.incomeNumber = `INC-${year}-${String(count + 1).padStart(4, '0')}`;
    } catch (error) {
    }
  }
});
...
IncomeSchema.pre('save', function() {
  // Check if items exist (detailed income)
  ...
  // Check if amount is provided (simple income)
